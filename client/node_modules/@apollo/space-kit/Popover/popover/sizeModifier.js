...
function findPlacementWithMinimumVerticalOverflow(_a) {
  var placementOverflows = _a.placementOverflows,
      popperRect = _a.popperRect,
      _b = _a.preventOverflow,
      preventOverflow = _b === void 0 ? {
    x: 0,
    y: 0
  } : _b;
  ...
  placementCalculations.sort(function (a, b) {
    ...
    var aPlacementWeight = placementOverflows.findIndex(function (overflow) {
      return overflow.placement === a.placement;
    });
    var bPlacementWeight = placementOverflows.findIndex(function (overflow) {
      return overflow.placement === b.placement;
    });
...
function buildPlacementsList(_a) {
  var state = _a.state,
      options = _a.options;
  /**
   * Preferred placement
  ...
  var fallbackPlacements = options.fallbackPlacements || (isBasePlacement || !options.flipVariations ? [getOppositePlacement(preferredPlacement)] : getExpandedFallbackPlacements(preferredPlacement));
  /**
  ...
  return _tslib.__spreadArrays([preferredPlacement], fallbackPlacements).reduce(function (acc, placement) {
    return acc.concat(getBasePlacement(placement) === "auto" ? computeAutoPlacement(state, _tslib.__assign(_tslib.__assign({}, options), {
      placement: placement
    })) : placement);
  }, []);
}
...
function getPlacementAndMaxSize(modifierArguments) {
  var state = modifierArguments.state,
      options = modifierArguments.options;
  var placements = buildPlacementsList(modifierArguments);
  /**
  ...
  var placementOverflows = placements.map(function (placement) {
    return {
      placement: placement,
      overflow: detectOverflow(state, _tslib.__assign(_tslib.__assign({}, options), {
        placement: placement
      }))
    };
  });
...
var sizeModifier = {
  ...
  fn: function (modifierArguments) {
    var _a = getPlacementAndMaxSize(modifierArguments),
        placement = _a.placement,
        maxSize = _a.maxSize; // Set the max height to be written in the `write` phase. See
    ...
    if (modifierArguments.state.placement !== placement && !modifierArguments.state.modifiersData[modifierArguments.name]._skip) {
      modifierArguments.state.modifiersData[modifierArguments.name]._skip = true;
      modifierArguments.state.placement = placement; // If we're applying a new position, then all other modifiers need to be
