...
export default class Fetch {
            ...
            case 'follow':
                ...
                if (!body && this.request.body) {
                    ...
                    if (this.request.bodyUsed) {
                        throw new DOMException('It is not possible to pipe a body after it is used.', DOMExceptionNameEnum.networkError);
                    }
                ...
                if (nodeResponse.statusCode !== 303 && this.request.body && !this.request._bodyBuffer) {
                    this.finalizeRequest();
                    this.reject(new DOMException('Cannot follow redirect with body being a readable stream.', DOMExceptionNameEnum.networkError));
                    return true;
                }
                if (this.request.signal.aborted) {
                    this.abort();
    ...
    prepareRequest() {
        ...
        if (this.request.referrer && this.request.referrer !== 'no-referrer') {
            this.request._referrer = FetchRequestReferrerUtility.getSentReferrer(this.ownerDocument, this.request);
        }
        else {
    ...
    validateRequest() {
        if (!SUPPORTED_SCHEMAS.includes(this.request._url.protocol)) {
            throw new DOMException(`Failed to fetch from "${this.request.url}": URL scheme "${this.request._url.protocol.replace(/:$/, '')}" is not supported.`, DOMExceptionNameEnum.notSupportedError);
        }
    }
    /**
